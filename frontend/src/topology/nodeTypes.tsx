...
function NodeHandles({ data }: { data: NodeData }) {
  const targetHandles = data.handles?.targets ?? ["top"];
  const sourceHandles = data.handles?.sources ?? ["bottom-0"];
  const spreadLeft = (idx: number, count: number) => `${((idx + 1) * 100) / (count + 1)}%`;
  return (
    <>
      {targetHandles.map((id, idx) => (
        <Handle
          key={id}
          id={id}
          type="target"
          ...
          style={{
            left: spreadLeft(idx, targetHandles.length),
            width: 8,
            height: 8,
            background: "#5c86b2",
      ...
      {sourceHandles.map((id, idx) => (
        <Handle
          ...
          style={{
            left: spreadLeft(idx, sourceHandles.length),
            width: 8,
            height: 8,
            background: "#5c86b2",
            border: "1px solid #8fb6de",
...
// ── Trunk downstream host (e.g. Proxmox) ─────────────────────────────────────
export function TrunkHostNode({ data }: NodeProps) {
  const d = data as NodeData;
  const w = d.width ?? 160;
  const h = d.height ?? 52;
  return (
...
// ── Multi-client access group on one port ─────────────────────────────────
export function PortGroupNode({ data }: NodeProps) {
  const d = data as NodeData;
  const w = d.width ?? 170;
  const h = d.height ?? 48;
  return (
